		...
		def make_lp2script_disconnect_mod():
			def _disconnect_mod():
				if hasattr(lp2script._modes, '_original_layer'):
					if lp2script._modes.selected_mode == u'mod_mode':
						lp2script._modes.selected_mode = u'user_2_mode'
					lp2script._modes.layer = lp2script._modes._original_layer
				if u'mod_mode' in lp2script._modes._mode_list:
					del lp2script._modes._mode_list[lp2script._modes._mode_list.index(u'mod_mode')]
				if u'mod_mode' in lp2script._modes._mode_map.keys():
					del lp2script._modes._mode_map[u'mod_mode']
...
class LaunchpadMk2ModHandler(FrameworkModHandler):
	...
	def __init__(self, register_component=None, *a, **k):
		...
		self.nav_box._on_off_values = (u'Mixer.Solo.Off', u'Mixer.Arm.On')
