...
def _Launchpad_Pro_setup_mod(args):
	...
	if not hasattr(lpproscript, '_modified') or (hasattr(lpproscript, '_modified') and not lpproscript._modified):
		...
		lpproscript.modhandler.layer = Layer(grid = lpproscript._midimap[u'Main_Button_Matrix'].submatrix[:,:],
		...
		lpproscript.modhandler.legacy_shift_layer = AddLayerMode(lpproscript.modhandler, Layer(nav_matrix = lpproscript._midimap[u'Main_Button_Matrix'].submatrix[2:6, 2:6]))

		modscript.schedule_message(1, modscript._device_provider.restart_mod)

		lpproscript._note_modes.add_mode(u'mod_mode', [ partial(lpproscript._layout_setup, consts.SESSION_LAYOUT_SYSEX_BYTE), lpproscript.modhandler])
		...
		def make_lpproscript_disconnect_mod():
			def _disconnect_mod():
				...
				if u'mod_mode' in lpproscript._note_modes._mode_list:
					del lpproscript._note_modes._mode_list[lpproscript._note_modes._mode_list.index(u'mod_mode')]
...
class LaunchpadProModHandler(FrameworkModHandler):
	...
	def __init__(self, register_component=None, *a, **k):
		...
		self.nav_box._on_off_values = (u'Mixer.SoloOff', u'Mixer.ArmOn')
		self._launchmodColors = range(128)
		launchmod_colors = [3, 13, 37, 53, 5, 21, 49]
		self._launchmodColors[1:127] = [launchmod_colors[x%7] for x in xrange(126)]
	...
	def _receive_grid(self, x, y, value = -1, identifier = -1, channel = -1, *a, **k):
		...
		if mod and self._grid_value.subject:
			if mod.legacy:
				x = x-self.x_offset
				y = y-self.y_offset
			if x in xrange(8) and y in xrange(8):
