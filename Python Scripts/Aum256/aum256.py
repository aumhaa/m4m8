...
class Aum256ModHandler(ModHandler):
	...
	def __init__(self, *a, **k):
		...
		self._push_colors[1:127] = [(x%7)+1 for x in xrange(126)]
	...
	def _receive_grid(self, x, y, value = -1, identifier = -1, channel = -1, *a, **k):
		...
		if mod and self._grid_value.subject:
			if mod.legacy:
				x = x-self.x_offset
				y = y-self.y_offset
			if x in xrange(16) and y in xrange(16):
...
class Aum256(ControlSurface):
	...
	def _setup_controls(self):
		...
		self._key = [MonoButtonElement(is_momentary = is_momentary, msg_type = MIDI_NOTE_TYPE, channel = 2, identifier = index, name = 'Key_' + str(index), script = self, monobridge = self._monobridge, skin = self._skin, color_map = COLOR_MAP, optimized_send_midi = optimized, resource_type = resource) for index in xrange(8)]
		for button in self._pad:
			button.set_color_map(tuple(range(128)))
		self._key_matrix = ButtonMatrixElement(name = 'KeyMatrix', rows = [self._key])

		self._shift = MonoButtonElement(is_momentary = is_momentary, msg_type = MIDI_NOTE_TYPE, channel = 2, identifier = 8, name = 'Shift', script = self, monobridge = self._monobridge, skin = self._skin, color_map = COLOR_MAP, optimized_send_midi = optimized, resource_type = resource)
		self._shift.set_color_map(tuple(range(128)))
		self._alt = MonoButtonElement(is_momentary = is_momentary, msg_type = MIDI_NOTE_TYPE, channel = 2, identifier = 9, name = 'Alt', script = self, monobridge = self._monobridge, skin = self._skin, color_map = COLOR_MAP, optimized_send_midi = optimized, resource_type = resource)
