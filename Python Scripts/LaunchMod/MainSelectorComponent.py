...
"""
class MainSelectorComponent(ModesComponent):


	def __init__(self, matrix, top_button, side_buttons, config_button):
		...
		ModesComponent.__init__(self)
		self._session_ring = SessionRingComponent(matrix.width(),  matrix.height())
		self._session = SpecialSessionComponent(session_ring = self._session_ring)
		self._zooming = SessionOverviewComponent(session_ring = self._session_ring, enable_skinning = True)
...
class MainSelectorComponent(ModeSelectorComponent):
	...
	def set_mode(self, mode):
		...
		self._mode_index = mode if self._mode_index != mode or mode == MIXER_MODE else self._mode_index
		self.update()
	...
	def _init_session(self):
		self._session._stop_clip_value = AMBER_THIRD
		self._session._stop_clip_triggered_value = AMBER_BLINK
		for scene_index in range(self._matrix.height()):
			scene = self._session.scene(scene_index)
			scene._triggered_value = GREEN_BLINK
			scene.name = u'Scene_' + str(scene_index)
			for track_index in range(self._matrix.width()):
				clip_slot = scene.clip_slot(track_index)
				clip_slot._triggered_to_play_value = GREEN_BLINK
				clip_slot._triggered_to_record_value = RED_BLINK
				clip_slot._stopped_value = AMBER_FULL
				clip_slot._started_value = GREEN_FULL
				clip_slot._recording_value = RED_FULL
