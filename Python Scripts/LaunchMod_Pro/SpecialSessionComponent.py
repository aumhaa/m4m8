...
import Live
from ableton.v2.base.util import find_if, in_range
from ableton.v2.base import depends, listens, listens_group
from ableton.v2.control_surface import Component
from ableton.v2.control_surface.control import ButtonControl
from ableton.v2.control_surface.components import SessionComponent, SessionOverviewComponent, SceneComponent, ClipSlotComponent
...
class SpecialSessionComponent(SessionComponent):
	...
	def _on_stop_scene_value(self, value, button):
		if self.is_enabled():
			if value is not 0 or not button.is_momentary():
				scene_index = list(self._stop_scene_clip_buttons).index(button) + self._session_ring.scene_offset
	...
	def _on_fired_slot_index_changed(self, track_index):
		button_index = track_index - self._session_ring.tracks_offset
...
class SpecialSessionZoomingComponent(SessionOverviewComponent):
	u"""
...
class SessionZoomingManagerComponent(Component):
