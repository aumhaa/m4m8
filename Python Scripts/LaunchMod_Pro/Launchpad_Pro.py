...
class MidiMap(SpecialMidiMap):
	...
	def with_shift(self, button_name):
		return ComboElement(self[button_name], modifier=[self[u'Shift_Button']],) # name=u'Shifted_' + button_name)
	

	def with_session_button(self, button):
		return ComboElement(button, modifier=[self[u'Session_Mode_Button']],) #  name=button.name + u'_With_Session_Button')
...
class Launchpad_Pro(IdentifiableControlSurface):
	...
	def _create_session(self):
		...
		self._session_navigation.layer = Layer(left_button=self._midimap[u'Arrow_Left_Button'], right_button=self._midimap[u'Arrow_Right_Button'], up_button=self._midimap[u'Arrow_Up_Button'], down_button=self._midimap[u'Arrow_Down_Button'])
		...
		self._session_zoom = SessionZoomingComponent(self._session_ring, name=u'Session_Overview', is_enabled=True, enable_skinning=True)
	

	def _create_recording(self):
		self._session_record = SpecialSessionRecordingComponent(target_track_component = self._target_track_component, name=u'Session_Recording', is_enabled=False, layer=Layer(record_button=self._midimap[u'Session_Record_Button']))
	...
	def _create_session_mode(self):
		self._modes.add_mode(u'session_mode', [partial(self._layout_setup, consts.SESSION_LAYOUT_SYSEX_BYTE), self._session_layer_mode, self._session_navigation, self._session.update_navigation_buttons], behaviour=CancelingReenterBehaviour(u'session_zooming_mode'))
	...
	def _create_note_modes(self):
		...
		self._note_modes.add_mode(u'mod', [partial(self._layout_setup, consts.SESSION_LAYOUT_SYSEX_BYTE), self.modhandler])
	...
	def _select_note_mode(self):
		...
		if not (self._note_modes.selected_mode is u'mod' and self.modhandler.is_locked()):
			if track is None or track.is_foldable or track in self.song.return_tracks or track == self.song.master_track or track.is_frozen:
				...
				self._note_modes.selected_mode = u'mod'
			...
			self._modes.update()
			if self._note_modes.selected_mode == u'audio_mode' or self._note_modes.selected_mode == u'mod':
				self.release_controlled_track()
			else:
