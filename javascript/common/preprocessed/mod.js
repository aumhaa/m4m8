	...
	for(var i=0;i<number_children;i++)
	{
		...
		for(var item in properties)
		{
			...
			if(this.MONOMODULAR.test(properties[item])>0)
			{
				...
				{
					this.modFunctions[func] = this.make_func(this.modFunctions[func]);
				}
				if(this.legacy)
				{
...
ModComponent.prototype.anything = function()
{
	...
	if(this.finder == null)
	{
		...
		if(DEBUG){post('added:', this.stored_messages[0], '\n');}
	}
}

ModComponent.prototype.list_functions = function()
{
	outlet(1, 'available_functions', this.modFunctions);
}

ModComponent.prototype.list_addresses = function()
{
	outlet(1, 'available_addresses', this.modAddresses);
}
...
ModComponent.prototype.send_stored_messages = function()
{
	...
	for(var index in this.stored_messages)
	{
		...
		if(this.stored_messages[index][0] in this)
		{
			...
			this[this.stored_messages[index][0]].apply(this, (this.stored_messages[index][1]));
		}
	}
}
...
//needs to be overridden in super
function notify_init(){}

var includes = new Global('includes');
includes.mod = ModComponent;