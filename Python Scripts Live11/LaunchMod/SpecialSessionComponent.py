			...
			if not button is None:
				...
				if 0 <= track_index < len(tracks_to_use):
					...
					if track.fired_slot_index == -2:
						button.set_light('Session.StopSingleClipTriggered')
					elif track.playing_slot_index >= 0:
						button.set_light('Session.StopSingleClip')
					else:
						button.set_light('Session.StopClipButton')
				else:
					button.send_value(4)
	

	def _update_stop_all_clips_button(self):
		self._stop_all_button and self._stop_all_button.set_light('Session.StopAllClips')
