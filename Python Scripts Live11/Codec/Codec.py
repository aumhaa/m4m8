...
from ableton.v2.control_surface import ControlSurface, ControlElement, Component, PrioritizedResource, Skin, DeviceBankRegistry, Layer
...
NORMALENCODER = (240, 0, 1, 97, 4, 30, 0o2, 00, 247)
FASTENCODER = (240, 0, 1, 97, 4, 30, 0o4, 00, 247)
...
def special_parameter_banks(device, device_dict = DEVICE_DICT):
	...
	if device != None:
		if device.class_name is 'LegacyModDeviceProxy':
			...
			return group([i for i in flatten(list(map(names_to_params, device_dict[device.class_name])))], 32)
		else:
			if device.class_name in MAX_DEVICES:
				try:
					banks = device.get_bank_count()
...
class CodecMixerComponent(MixerComponent):
	...
	def set_return_controls(self, controls):
		...
		for control, channel_strip in zip(controls or [], self._return_strips):
...
class CodecModHandler(ModHandler):
	...
	def __init__(self, *a, **k):
		...
		self.nav_box = NavigationBox(self, 16, 16, 2, 2, self.set_offset)
		...
		if self.is_enabled() and self._active_mod and not self._local:
			...
			if not grid is None:
				...
				def xiterbuttons(matrix):
					for i, j in product(range(matrix.width()), range(matrix.height())):
