	...
	def send_value(self, value, force = False):
		...
		if(type(self) != type(None)):
			...
			if (force or self._force_next_send or ((value != self._last_sent_value) and self._is_being_forwarded)):
				...
				debug('flash_state:', math.floor((value -1)/self._num_colors), my_round((value -1)/self._num_colors))
...
class MonolooperComponent(Component):
...
class DeviceControlComponent(Component):
