...
from Push2.model.declaration import Binding, custom_property, id_property, listmodel, listof, view_property, ViewModel, ModelVisitor
from Push2.model.repr import BrowserItemAdapter, BrowserListWrapper, ClipAdapter, DeviceAdapter, DeviceParameterAdapter, EditModeOptionAdapter, ItemListAdapter, ItemSlotAdapter, LiveDialogAdapter, OptionsListAdapter, SimplerDeviceAdapter, TrackAdapter, TrackControlAdapter, TrackListAdapter, VisibleAdapter
...
class Device(Binding):
	...
	id = id_property()
	class_name = view_property(str, '')
	icon = view_property(str, '')
...
class EditModeOption(Binding):
	...
	firstChoice = view_property(str, '')
	secondChoice = view_property(str, '')
...
class MixerSelectionListModel(Binding):
	...
	visible = view_property(bool, False)
	items = view_property(listof(ItemSlotModel))
	selectedItem = view_property(str, '')
...
class DeviceParameter(Binding):
	...
	name = view_property(str, '')
	original_name = view_property(str, '')
	min = view_property(float, 0.0)
	max = view_property(float, 0.0)
	value = view_property(float, 0.0)
	valueItems = view_property(listof(str))
	displayValue = view_property(str, '')
	unit = view_property(str, '')
	id = id_property()
...
class SimplerProperties(Binding):
	...
	playhead_real_time_channel_id = view_property(str, '')
	waveform_real_time_channel_id = view_property(str, '')
...
class RealTimeChannel(Binding):
	channel_id = view_property(str, '')
	object_id = view_property(str, '')
...
class BrowserModel(Binding):
	...
	load_text = view_property(str, '')
	prehear_enabled = view_property(bool, False)
	context_text = view_property(str, '')
...
class Notification(Binding):
	visible = view_property(bool, False)
	message = view_property(str, '')


class RealTimeClient(Binding):
	clientId = view_property(str, '')


class ConvertModel(Binding):
	ADAPTER = VisibleAdapter
	source_color_index = view_property(int, -1)
	source_name = view_property(str, '')
	visible = view_property(bool, False)
	available_conversions = view_property(listof(str))
...
class ScalesModel(Binding):
	ADAPTER = VisibleAdapter
	visible = view_property(bool, False)
	scale_names = view_property(listof(str), '')
	selected_scale_index = view_property(int, -1)
	root_note_names = view_property(listof(str), '')
...
class QuantizeSettingsModel(Binding):
	...
	quantization_option_names = view_property(listof(str))
...
class FixedLengthSettingsModel(Binding):
	option_names = view_property(listof(str))
	selected_index = view_property(int, -1)
...
class ModeState(Binding):
	main_mode = view_property(str, '')
	mix_mode = view_property(str, '')
	global_mix_mode = view_property(str, '')
	device_mode = view_property(str, '')


class MixerRealTimeMeterModel(Binding):
	real_time_meter_channel_ids = view_property(listof(str), '')
...
class GeneralSettingsModel(Binding):
	workflow = view_property(str, 'scene')
...
class SetupModel(Binding):
	...
	selected_mode = view_property(str, '')
	modes = view_property(listof(str))
...
class ValueModel(Binding):
	ADAPTER = VisibleAdapter
	visible = view_property(bool, False)
	value_string = view_property(str, '')
...
class FirmwareUpdateModel(Binding):
	ADAPTER = VisibleAdapter
	visible = view_property(bool, False)
	firmware_file = view_property(str, '')
	data_file = view_property(str, '')
	state = view_property(str, '')


class LiveDialogViewModel(Binding):
	ADAPTER = LiveDialogAdapter
	visible = view_property(bool, False)
	text = view_property(str, '')
...
class RootModel(ViewModel):
	...
	#trollMixerSelectView = view_property(TrollMixerSelectionListModel)
	#trollMixerView = view_property(TrollMixerViewModel)


import json
#from _model import RootModel
from Push2.model.generation import generate_mrs_model
