...
from ableton.v2.base.util import const, mixin, recursive_map
...
from ableton.v2.control_surface.mode import LayerMode, AddLayerMode, ModeButtonBehaviour
...
class Launchpad_MK2(IdentifiableControlSurface, ControlSurface):
	...
	def _create_controls(self):
		...
		self._session_button_single = make_button(108, 0, 'Session_Mode_Button', msg_type=MIDI_CC_TYPE, is_modifier=True, skin = self._skin)
		self._session_button = FilteringMultiElement([self._session_button_single] + [ make_button(108, channel, skin = self._skin, msg_type=MIDI_CC_TYPE, name='Session_Mode_Button_ch_%d' % (channel,)) for channel in multi_button_channels ], feedback_channels=[0])
	...
	def _create_user_controls(self):
		...
		self._user_2_matrix = ButtonMatrixElement(rows=[ [ make_button(offset + col_index, USER_2_CHANNEL, name='User_2_Button_%d_%d' % (col_index, row_index)) for col_index in range(8) ] for row_index, offset in enumerate(range(81, 10, -10)) ], name='User_2_Matrix')
	...
	def _create_session(self):
		self._session_ring = SessionRingComponent(num_tracks=self._session_matrix.width(), num_scenes=self._session_matrix.height(), name = 'Session_Ring')
		self._session = SessionComponent(session_ring = self._session_ring, name='Session', is_enabled=False, auto_name = True)
		#layer=Layer(track_bank_left_button=self._left_button, track_bank_right_button=self._right_button, scene_bank_up_button=self._up_button, scene_bank_down_button=self._down_button))
		...
		self._session_zoom = SessionOverviewComponent(self._session_ring, is_enabled=False)
		#layer=Layer(nav_up_button=with_modifier(self._session_button_single, self._up_button), nav_down_button=with_modifier(self._session_button_single, self._down_button), nav_left_button=with_modifier(self._session_button_single, self._left_button), nav_right_button=with_modifier(self._session_button_single, self._right_button), button_matrix=self._session_zoom_matrix))
	...
	def _create_mixer(self):
		self._mixer = MixerComponent(tracks_provider = self._session_ring, is_enabled=False, name='Mixer')
	...
	def _create_modes(self):
		self._modes = NotifyingModesComponent()
